...
- [12. 快速参考卡片（按场景分类）](#12-快速参考卡片按场景分类)
- [13. 常见坑](#13-常见坑)
- [14. 命令速查索引](#14-命令速查索引)
- [15. 进阶学习路径推荐](#15-进阶学习路径推荐)
...
- **破坏性命令先干跑**：`rm` / `mv` / `find -delete` 之前先用 `ls`、`echo` 把命令打印出来确认；路径以 `-` 开头要加 `--` 终止选项解析。
- **管道吞错误**：脚本里默认管道只反映最后一条命令的结果，加 `set -o pipefail` 才能捕获前段失败。
- **空格与通配符**：变量一律加双引号（`"$f"`）；通配符先 `ls *.log` 看命中范围再执行。
- **`find -exec` 与 `xargs`**：文件名含空格会断裂，用 `-print0` 配 `xargs -0`。
- **`tar` 目录穿越**：解压前先 `tar -tf` 看清单，避免恶意路径覆盖系统文件。
- **磁盘满排查链**：`df -h` → `du -sh * \| sort -h` 逐层定位 → 确认后再删。
- **杀进程**：`kill` 默认 SIGTERM 给进程清理机会，`kill -9` 是最后手段；杀不掉的僵尸进程要找其父进程回收。
- **shell 类型差异**：登录 shell 与非登录 shell 加载的配置文件不同（`.bash_profile` vs `.bashrc`），脚本里别依赖交互式配置。
- **时间/时区**：跨机对日志用 `date -u` 与 `timedatectl` 统一时区，避免"时间线错乱"。
...
下一篇：《02-tcpdump网络抓包.md》　｜　模块索引：《../README.md》

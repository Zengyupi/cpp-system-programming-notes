...
| 需求 | 做法 |
| --- | --- |
| 二进制打包 | `struct.pack("!BHI", 1, 2, 3)`（网络序）与 `unpack` 解析 |
| 字段拼接 | 长度字段最后回填：先按占位打包，`struct.pack_into` 写入长度 |
| 校验和 | 简单的 16 位反码和 / CRC32（`zlib.crc32`）实现 |
| 变异策略 | 位翻转、字节替换、插入/删除、边界值（0、1、max、-1）、长度字段篡改 |
| 语料管理 | 从真实抓包导出种子存目录；每轮把新覆盖的用例入库 |
| hypothesis | `@given(st.binary(min_size=1))` 自动生成并缩减失败用例 |
| 结果校验 | 被测进程崩溃 = 发现 bug；超时也算异常（挂死） |
| 崩溃归档 | 保存触发 crash 的输入 + 命令 + 栈回溯，命名含哈希 |
| 大流量控制 | 限速/限量（`--max-cases`），避免 CI 时间失控 |
| 与 C++ 配合 | C++ 侧保证"任意输入不崩"是 Fuzz 的目标；Python 只负责生成与判定 |
...
上一篇：《02-测试驱动与对拍器.md》　｜　下一篇：《04-调试与性能分析辅助.md》　｜　模块索引：《../README.md》

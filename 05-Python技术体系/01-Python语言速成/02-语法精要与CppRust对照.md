...
| 需求 | 做法 |
| --- | --- |
| 变量即引用 | `a=[1]; b=a; b.append(2)` → `a` 也变；需 `copy.copy` / `deepcopy` |
| 身份 vs 相等 | `is` 比身份、`==` 比值；小整数/短字符串有驻留（interning） |
| 整除与浮点 | `//` 向下取整、`/` 真除；`0.1+0.2 != 0.3` 用 `math.isclose` |
| 字符串 | 不可变；`str`(Unicode) vs `bytes`；`f"{x:>5}"` 格式化 |
| 容器 | `list`(可变有序) / `tuple`(不可变) / `dict`(有序 hash) / `set`；推导式一行建表 |
| 切片 | `a[1:5:2]`；负索引 `a[-1]`；切片产生**拷贝**（非视图） |
| 控制流 | `for x in iterable`；`for...else`；少用 `range(len(a))` |
| 真值测试 | 空容器/`0`/`None`/`""` 为假；用 `if x:` 而非 `if x == []` |
| 类型 | 动态；`isinstance(x, T)` 判断；注解不强制运行期检查 |
| 常见坑 | 可变默认参数；闭包延迟绑定；浮点精度；缩进混用 tab/space |
...
上一篇：《01-环境工具链与运行模型.md》　｜　下一篇：《03-函数模块与包.md》　｜　模块索引：《../README.md》
